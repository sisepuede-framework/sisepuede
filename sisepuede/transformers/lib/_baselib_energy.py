...
def transformation_entc_renewable_target(
    ...
    **kwargs
    ...
    """Implement the "% of electricity is generated by renewables" 
        transformation. Applies to both renewable (true renewable) and fossil
        fuel (fake renewable) transformations
    ...
    df_input : pd.DataFrame
        Input data frame containing baseline trajectories
    magnitude_target : Union[float, str]
        (1) Magnitude of target to hit by final time period; OR   
        (2) Optional str "VEC_FIRST_RAMP", which will set the magnitude to the 
            mix of renewable capacity at the first time period where 
            VEC_FIRST_RAMP != 0
    vec_ramp : np.ndarray
        Implementation ramp vector
    model_enerprod : EnergyProduction
        EnergyProduction model used to define variables. Also defines the 
        internal model_atrributes variable to preserve logical consistency
    ...
    if scale_non_renewables_to_match_surplus_msp:
        ...
        # cap non-renewables at less than the renewable target
        field_re_targ = model_attributes.get_variable(
            model_enerprod.modvar_enfu_nemomod_renewable_production_target
        )
        field_re_targ = field_re_targ.fields[0]

        # bound the surplus by 1 - the renewable energy target
        vec_re_targ = df_out[field_re_targ].to_numpy()
        bounds = [(0.0, max(min(1.0 - x, 1.0), 0.0)) for x in vec_re_targ]
        vec_entc_msp_surplus = sf.vec_bounds(
            vec_entc_msp_total_mass_original - vec_entc_msp_total_mass_no_drops, 
            bounds,
            # (0.0, 1.0)
            # CHANGED HEREHEREHERE 20251104--added bound based on renewable energy target
