...
var_to_save = "frac_inen_energy_agriculture_and_livestock_gas_petroleum_liquid"
...
for var_group in all_var_prefix_group:
    if var_group != non_zero_variable : 
        print(var_group)
...
# Take the value of frac_inen_energy_agriculture_and_livestock_gas_petroleum_liquid from fake data 
all_group_historical[non_zero_variable] = fake_data[non_zero_variable].unique()[0]
all_group_projected[non_zero_variable] = fake_data[non_zero_variable].unique()[0]

## Sort dataframes
all_group_historical.sort_values(by = ["Nation", "Year"], inplace = True) 
all_group_projected.sort_values(by = ["Nation", "Year"], inplace = True) 
all_group_projected.reset_index(drop = True, inplace = True)

## Renormalize variables of frac_inen_energy_agriculture_and_livestock

total_historical = all_group_historical[all_var_prefix_group].sum(1)
total_projected = all_group_projected[all_var_prefix_group].sum(1)

for var_group in all_var_prefix_group:
    all_group_historical[var_group] = all_group_historical[var_group]/total_historical
    all_group_projected[var_group] = all_group_projected[var_group]/total_projected

## Save dataframes with renormalize values
for var_group in all_var_prefix_group:
    all_group_historical[["Nation", "iso_code3", "Year",var_group]].to_csv(os.path.join(historical_to_save_path, f"{var_group}.csv"), index = False)
    all_group_projected[["Nation", "iso_code3", "Year",var_group]].to_csv(os.path.join(projected_to_save_path, f"{var_group}.csv"), index = False)
